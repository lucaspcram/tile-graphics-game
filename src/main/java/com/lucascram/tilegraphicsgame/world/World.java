...
public class World {
	...
	private void spawnGoal() {
		while(true) {
			...
			int y = RandomUtility.randIntInRange(0, WORLD_HEIGHT);
			if(map.isValidTile(x, y)) {
				if(map.isTileXYDirt(x, y) && x != beetleEntity.getXPos() && y!= beetleEntity.getYPos()) {
	...
	public void updateWorld(int newBeetleX, int newBeetleY) {
		...
		//check for win
		if(beetleEntity.getXPos() == goalEntity.getXPos() && beetleEntity.getYPos() == goalEntity.getYPos()) {
			incrementScore();
			generateWorld(false);
