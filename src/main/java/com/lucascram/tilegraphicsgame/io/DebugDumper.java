...
public class DebugDumper {
	...
	public static void handleException(String path, String errMessage, StackTraceElement[] stack, Exception e) {
		if(path == null) {
			JOptionPane.showMessageDialog(null, errMessage, TileGraphicsGame.TITLE + " Error", JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
			dumpToFile(DebugDumper.DEBUG_FILE_PATH, e.getStackTrace());
			System.exit(1);
		} else {
			JOptionPane.showMessageDialog(null, errMessage + ": " + path, TileGraphicsGame.TITLE + " Error", JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
			dumpToFile(DebugDumper.DEBUG_FILE_PATH, e.getStackTrace());
			System.exit(1);
		}
	...
	private static void dumpToFile(String path, StackTraceElement[] stack) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Debug file error", TileGraphicsGame.TITLE + " Error", JOptionPane.ERROR_MESSAGE);
			System.exit(1);
		}
